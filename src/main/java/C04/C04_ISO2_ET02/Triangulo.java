...
public class Triangulo {
	...
	// o imposibles donde de verdad sabremos si lo que hacen es correcto es en el
	...
	public String[] devolverTipo() {
		// Primer valor para ver su tipo dependiendo de sus lados y el segundo depende
		// de sus ángulos.
		String tipos[] = new String[2];
	...
	public String clasificarLado() {
		// Lanzar excepción si la raiz de los catetos al cuadrado es diferente de la
		// hipotenusa.
		String tipoLado = "";
	...
	public String clasificarAngulo() {
		...
		for (int i = 0; i < 3; i++) {
			if (this.angulos[i] > anguloMayor)
				anguloMayor = this.angulos[i];
		}
	...
	public String verificarTriangulo() {
		...
		try {
			for (int i = 0; i < 3; i++) {
				if (this.lados[i] <= 0)
					throw new LadoInvalidoException("Existe un lado que es menor o igual a 0.");
			}
		...
		try {
			for (int i = 0; i < 3; i++) {
				...
				if (this.angulos[i] <= 0)
					throw new AnguloInvalidoException("El ángulo es menor o igual a 0.");
			}
	...
	public boolean verificarTrianguloTeoremaSeno() {
		// El teorema del seno es la mejor forma de verificar si un triángulo es posible
		// o no
		// Para poder hacerlo usando estimaciones es necesario sacar valores inferiores
		// y superiores
		// y ver si el valor está entre estos. El valor varía mucho en función de la
		// cercanía/lejanía
		// con respecto a pi/2 o 90º, por lo que el margen de error en algunas ocasiones
		// es demasiado
		...
		// Primero truncamos el valor de los angulos a 2 decimales que es sobre lo que
		// trabajaremos
		for (int i = 0; i < 3; i++) {
		...
		// Calculamos los valores extremos para poder estimar, como en 90º está el
		...
		for (int i = 0; i < 3; i++) {
			if (angulosRedondeados[i] >= 90.00) {
				if ((angulosRedondeados[i] - MARGENERROR) < 90.00) {
					...
					teoremaSenoExtremos[i][0] = this.lados[i]
							/ Math.sin(Math.toRadians(angulosRedondeados[i]) - MARGENERROR);
				}
				teoremaSenoExtremos[i][1] = this.lados[i]
						/ Math.sin(Math.toRadians(angulosRedondeados[i]) + MARGENERROR);
			} else {
				if ((angulosRedondeados[i] + MARGENERROR) > 90.00) {
					teoremaSenoExtremos[i][0] = lados[i] - MARGENERROR;
		...
		// Finalmente, se realiza la comparación, en caso de que algún valor no coincida
		// se evaluará en falso
		// También se usa el calcular el módulo junto con sumar 1 y 2 para poder
		// aplicarlo sin tener en cuenta el valor de i.
		for (int i = 0; i < 3; i++) {
			if (!(((constante[i] > teoremaSenoExtremos[(i + 1) % 3][0])
					&& (constante[i] > teoremaSenoExtremos[(i + 2) % 3][0]))
					&& ((constante[i] < teoremaSenoExtremos[(i + 1) % 3][1])
							&& (constante[i] < teoremaSenoExtremos[(i + 2) % 3][1])))) {
				valido = false;
			}
		}
		return valido;
